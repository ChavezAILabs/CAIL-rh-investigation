/- C-017 fix, hypothesis-use audit (standing check item 1).
   Scratch copy of chavez_transform_integrable with h_integrable DELETED from
   the signature. Result: FAILS -- unknown identifiers `h_integrable.norm`
   and `h_integrable` at the majorant-integrability and measurability steps.
   h_integrable is load-bearing. -/
...
-- h_integrable DELETED
theorem chavez_transform_integrable_no_hint
